...
redact_collected_session_secrets() {
	if [ ! -d "$AGENT_SESSIONS_DIR" ]; then
		return
	fi

	echo "Redacting secret-looking tokens from collected agent sessions..."
	find "$AGENT_SESSIONS_DIR" -type f -print0 | while IFS= read -r -d '' file; do
		python3 - "$file" <<'PY'
import pathlib
import re
import sys
...
openrouter_pattern = r"sk-" + r"or-v1-" + r"[A-Za-z0-9]+"
composio_pattern = r"ak_" + r"[A-Za-z0-9]{12,}"
redacted = re.sub(openrouter_pattern, "sk-" + "or-v1-REDACTED", data)
redacted = re.sub(composio_pattern, "ak_REDACTED", redacted)
redacted = re.sub(r"(OPENROUTER_API_KEY=)[^\\s\\\"']+", r"\\1REDACTED", redacted)
redacted = re.sub(r"(COMPOSIO_API_KEY=)[^\\s\\\"']+", r"\\1REDACTED", redacted)
